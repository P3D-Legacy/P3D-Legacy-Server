...
{
    public class NotifyPlayerLeaved : INotificationHandler<PlayerLeavedNotification>
    {
        private readonly IPlayerContainerReader _playerContainer;

        public NotifyPlayerLeaved(IPlayerContainerReader playerContainer)
        {
        ...
        public async Task Handle(PlayerLeavedNotification notification, CancellationToken ct)
        {
            ...
            {
                await player.NotifyAsync(new PlayerLeavedEvent(new EventPlayerModel(id, name, gameJoltId)), ct);
            }
        }
    }
}
