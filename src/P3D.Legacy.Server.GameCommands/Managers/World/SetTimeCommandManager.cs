...
namespace P3D.Legacy.Server.GameCommands.Managers.World
{
    public class SetTimeCommandManager : CommandManager
    {
        ...
        public override async Task HandleAsync(IPlayer client, string alias, string[] arguments, CancellationToken ct)
        {
            if (arguments.Length == 1)
            {
                if (TimeSpan.TryParseExact(arguments[0], "HH\\:mm\\:ss", null, out var time))
                {
                    ...
                    await SendMessage(client, $"Set time to {time}!", ct);
                    await SendMessage(client, "Disabled Real Time!", ct);
                }
                else
                    await SendMessage(client, "Invalid time!", ct);
            }
            else
        ...
        {
            await SendMessage(client, $"Correct usage is /{alias} <Time[HH:mm:ss]/Real>", ct);
        }
    }
}